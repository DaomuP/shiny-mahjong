...
const UNIT_EFFECT = {
  ...
  'Straylight': { key: 'reshuffle', name: '市场重置', desc: '明牌列洗牌重排后，从明牌列拿 1 张、再弃 1 张手牌' },
  'ALSTROEMERIA': { key: 'refresh', name: '手牌重整', desc: '任意张手牌放回牌山，摸相同张数' },
  'CoMETIK': { key: 'spy', name: '查验', desc: '查看一名对手手牌，自选 1 张拿走' },
...
async function applyUnitEffect(p, unit, i, card) {
  ...
  switch (key) {
    ...
    case 'reshuffle': {
      ...
      while (G.display.length < DISPLAY_SIZE) { const c = drawCards(1)[0]; if (!c) break; G.display.push(c); }
      sfx('swap');
      const picks = await pickCards('display', 1, true);
      if (!picks || !picks.length) break;
      const take = picks[0];
      const idx = G.display.indexOf(take);
      if (idx >= 0) G.display[idx] = drawCards(1)[0] || null;
      p.hand.push(take); sortHand(p); sfx('draw');
      const pay = (await pickCards('hand', 1, false) || [])[0];
      if (pay) { p.hand.splice(p.hand.indexOf(pay), 1); G.dark.push(pay); sfx('discard'); log(`${p.name} 从明牌列拿走 ${take.name}，弃掉 ${pay.name}`); }
      break;
    }
    case 'refresh': {
      let sel;
    ...
    case 'spy': {
      ...
      if (p.ai) {
        await sleep(1800);
        const ranked = target.hand.slice().sort((a, b) => cardValueFor(target, b) - cardValueFor(target, a));
        stolen = ranked[0] || null;
      } else {
        stolen = await spyCloseWait();
      }
      if (stolen) {
        target.hand.splice(target.hand.indexOf(stolen), 1);
        p.hand.push(stolen); sortHand(p);
        log(`${p.name} 从 ${target.name} 手牌拿走 ${stolen.name}`);
      }
      UI.spyView = null; render(); break;
    }
    case 'forcedswap': {
      let give;
...
function render() {
  ...
  $('#spyModal').innerHTML = UI.spyView ? `<div class="dlgbox"><h3>查验 · ${esc(UI.spyView.name)} 的手牌（点击一张拿走）</h3><div class="payrow">${UI.spyView.hand.map((c, k) => cardImg(c, 'mini2b spycard', 'spy', k)).join('')}</div><button data-act="spyClose">不拿了</button></div>` : '';
...
document.addEventListener('click', async e => {
  ...
  if (t.dataset.uid && t.dataset.side === 'spy' && UI.spyView) { const card = UI.spyView.hand[Number(t.dataset.idx)]; UI.spyView = null; const res = UI.spyCloseWait; UI.spyCloseWait = null; render(); if (res) res(card); return; }
  ...
  if (t.dataset.act === 'spyClose' && UI.spyView) { UI.spyView = null; const r = UI.spyCloseWait; UI.spyCloseWait = null; render(); if (r) r(null); return; }
